...
def train():
    ...
    for outer_iter in outer_pbar:
        ...
        # Save checkpoint after each outer iteration (overwrite)
        torch.save(
            network.state_dict(),
            models_dir / "deepcubea_heuristic_latest.pt",
        )
        ...
        # Save cumulative loss CSV after each outer iteration
        csv_path = imgs_dir / "loss_curve_latest.csv"
        with open(csv_path, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["epoch", "loss", "is_outer_start", "theta_updated"])
            outer_starts = set(outer_boundaries)
            theta_set = set(theta_update_points)
            for i, loss_val in enumerate(all_losses):
                writer.writerow([
                    i,
                    loss_val,
                    1 if i in outer_starts else 0,
                    1 if i in theta_set else 0,
                ])

        # Save cumulative loss curve after each outer iteration
        ...
        plt.tight_layout()
        plt.savefig(imgs_dir / "loss_curve_latest.png")
        plt.close()
