...
| `DeepCubeA 24-Puzzle 适配` | [🔗](./abstract/deepcubea-puzzle24-adaptation.md) | S0-S3 已完成，S4-S5 待执行；N=5，Option B 显式传参，数据/模型在 data/ 根目录，冒烟 J≈104/89 通过 | ⏳ |
...
| `DeepCubeA 24-Puzzle 适配` | [🔗](./plan/deepcubea-puzzle24-adaptation.md) | executing（S0-S3 已完成，S4-S5 待执行） |
...
- `06-14 22:22`: DeepCubeA 24-Puzzle 适配 S0-S3 完成 —— S0 数据预处理（pickle→numpy int64）、S1 config（N=5 + USE_OFFICIAL_DATA=True）、S2 Option B 显式传参、S3 冒烟 PASS（J=104/89）；修正：data/ 根目录不建子目录；wiki 计划/摘要/索引已同步
