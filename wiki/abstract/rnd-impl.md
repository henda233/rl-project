...
- 奖励公式：`r_total = r_ext + β · r_int_norm`，其中 `r_int_norm = r_int / (running_std + 1e-8)`（仅除标准差，不减均值）。β 线性衰减：`β_episode = max(RND_BETA_END, β_last - RND_BETA_DECAY)`，从 RND_BETA=100 衰减到 RND_BETA_END=1.0
...
>
> `config.py` RND 配置段新增：RND_HIDDEN_DIM=128、RND_OUTPUT_DIM=256、RND_LR=1e-3、RND_BETA=100、RND_BETA_END=1.0、RND_BETA_DECAY=0.05、RND_EPOCHS=5、RND_NUM_EPISODES=10000、RND_BUFFER_SIZE=5。
...
- **下游被依赖**：无
- **变更扩散评估**：低（ppo_rnd_agent.py 自包含，config.py 仅追加 RND 参数段，ppo_agent.py 仅修改输出目录。06-05 新增 β 线性衰减：current_beta 每 episode 递减 RND_BETA_DECAY，progress bar 新增 β 显示）
