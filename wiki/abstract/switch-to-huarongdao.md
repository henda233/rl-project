...
updated_at: 2026-06-10 16:20:00
---
...
> 执行 5 步计划完成切换：S1 清理 config.py（删除AC/再训练/推理/塑形/并行训练段，RND_HIDDEN_DIM→256）→ S2 env_digital_huarongdao.py 增强 info → S3 ppo_agent.py 破坏性改造（华容道环境、删除塑形/再训练、max_placed+steps监控、terminated通关、单图）→ S4 ppo_rnd_agent.py 破坏性改造（华容道环境、r_ext从实际reward获取、max_placed+steps监控、terminated通关）→ S5 WIKI 维护。
